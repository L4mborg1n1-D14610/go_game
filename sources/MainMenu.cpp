...
void MainMenu::if_delete_stones(bool&& color) {
				...
				if (color) {
								...
								for (std::list < std::pair<int, int>>::iterator it = list_coord_black_stones.begin();
												...
												int x = it->first;
												int y = it->second;
												if (check_neighbours(x, y, _color, color) && dont_eat_flag) {
																delete_flag = true;
																return;
												}
												else {
...
bool MainMenu::check_neighbours(int& x, int& y, bool& color, bool& last_color) {    //ïîìåíÿòü ïîòîì ïîðÿäîê if -  â íà÷àëå íàèáîëåå âåðîÿòíûå ñîáûòèÿ
